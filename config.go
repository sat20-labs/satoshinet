...
func loadConfig() (*config, []string, error) {
	...
	if len(cfg.MiningAddrs) != 0 {
		err := fmt.Errorf("%s: not support set mining address", funcName)
		fmt.Fprintln(os.Stderr, err)
		fmt.Fprintln(os.Stderr, usageMessage)
		return nil, nil, err
	}
	...
	if cfg.Generate {
		cfg.TxIndex = true
		if cfg.MiningPubKey != "" {
			// 只需要设置MiningPubKey，不需要设置地址。通过MiningPubKey生成挖坑地址，这个地址是跟服务节点的多签地址
			// 服务节点提供一层索引器数据服务，分享挖坑收益
			// 如果没有服务节点，也就是自己独立挖矿，不需要生成多签地址
			//if len(cfg.MiningAddrs) == 0 {
			...
			if !cfg.EnableSTP {
				str := "%s: the EnableSTP flag is not set and no mining pubkey"
				err := fmt.Errorf(str, funcName)
				fmt.Fprintln(os.Stderr, err)
				fmt.Fprintln(os.Stderr, usageMessage)
				return nil, nil, err
