...
func TestLastInputPreviousOutputInvokerResolverUsesPreviousOutputAddress(t *testing.T) {
	...
	lastAddr, err := btcutil.NewAddressPubKeyHash(repeatedBytes(20, 0x44), &chaincfg.TestNetParams)
...
func TestLastInputPreviousOutputInvokerResolverRejectsMissingPreviousOutput(t *testing.T) {
	tx := wire.NewMsgTx(2)
	tx.AddTxIn(&wire.TxIn{Witness: wire.TxWitness{[]byte{1}, testAgentPubKey(0x02)}})
...
func testAgentPubKey(prefix byte) []byte {
	pubKey := make([]byte, 33)
	pubKey[0] = prefix
