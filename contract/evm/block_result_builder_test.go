...
func TestBuildBlockResultTxsDeployInvoke(t *testing.T) {
	...
	result, err := BuildBlockResultTxs(BlockResultBuildRequest{
		...
		ResolveOutput: func(resultTx *wire.MsgTx) ([]ResultOutput, error) {
			return ResultOutputsFromTx(resultTx, TestnetContractPrefix, func(pkScript []byte) (string, bool, error) {
...
func TestContractUTXOOverlayIncludesAndSpendsBlockOutputs(t *testing.T) {
	...
	overlay := NewContractUTXOOverlay(TestnetContractPrefix, nil)
...
func blockResultInvokeTx(t *testing.T, contract ContractAddress, payload InvokePayload, gasAssetName string, gasAmount uint64) *wire.MsgTx {
	t.Helper()
	...
	tx.AddTxOut(wire.NewTxOut(0, wire.TxAssets{{
		Name:   *assetName,
		Amount: *scommon.NewDefaultDecimal(int64(gasAmount)),
	}}, contractScript))
	return tx
}
