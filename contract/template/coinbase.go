...
func FindCoinbaseStateRoot(tx *wire.MsgTx) (StateRootPayload, bool, error) {
	...
	for _, txOut := range tx.TxOut {
		...
		if found {
			return StateRootPayload{}, false, errors.New("multiple template state roots in coinbase")
		}
...
func VerifyCoinbaseStateRoot(tx *wire.MsgTx, expected [32]byte) error {
	...
	if !found {
		return errors.New("missing template state root in coinbase")
	}
	if payload.StateRoot != expected {
		return errors.New("template state root mismatch")
	}
	return nil
}
...
func UpsertCoinbaseStateRoot(tx *wire.MsgTx, root [32]byte) error {
	...
	script, err := contractcommon.StateRootNullDataScript(StateRootPayload{StateRoot: root})
	if err != nil {
		return err
	}
	foundIndex := -1
	for i, txOut := range tx.TxOut {
		...
		if foundIndex >= 0 {
			return errors.New("multiple template state roots in coinbase")
		}
		foundIndex = i
	}
	if foundIndex >= 0 {
