...
func (r *ContractRuntime) settleLimitOrders(height int64,
	...
	changed, err := applyInvalidItems(r.contract, &state, plan, height)
	if err != nil {
		return nil, err
	}
	...
	for i < len(buyIDs) && j < len(sellIDs) {
		...
		buyPriceString, err := limitOrderItemPrice(buy)
		if err != nil {
			return nil, err
		}
		sellPriceString, err := limitOrderItemPrice(sell)
		if err != nil {
			return nil, err
		}
		buyPrice := parseDecimalOrZero(buyPriceString)
		sellPrice := parseDecimalOrZero(sellPriceString)
		...
		if err := applyLimitOrderDeal(buy, sell, matchAmt, matchValue, sellPriceString); err != nil {
			return nil, err
		}
...
func (r *ContractRuntime) settleAMM(height int64,
	...
	if running.TradingReady {
		...
		if err != nil {
			return nil, fmt.Errorf("AMM pool gas: %w", err)
		}
...
func applyAMMLiquidity(state *TemplateRuntimeState, plan *SettlementPlan, foundationAddress string) (bool, error) {
	...
	poolGas, err := decimalInt64(running.AssetBInPool)
	if err != nil {
		return false, fmt.Errorf("AMM liquidity pool gas: %w", err)
	}
	...
	for i := range state.Items {
		...
		case OrderTypeAddLiquidity:
			...
			reserveAsset, reserveGas, leftAsset, leftGas, err := reserveAMMLiquidity(addAsset, addGas, poolAsset, poolGas, *running)
			if err != nil {
				return false, err
			}
		...
		case OrderTypeRemoveLiquidity:
			...
			remove, err := removeLiquidityItemAmount(item)
			if err != nil {
				return false, err
			}
			...
			depositValue, err := proportionalInt64(cost, remove, owned)
			if err != nil {
				return false, err
			}
...
func sortLimitOrders(items []InvokeItem, ids []int, buy bool) error {
	prices := make(map[int]*scommon.Decimal, len(ids))
	for _, id := range ids {
		if id < 0 || id >= len(items) {
			return fmt.Errorf("limit order index %d out of range", id)
		}
		price, err := limitOrderItemPrice(&items[id])
		if err != nil {
			return err
		}
		prices[id] = parseDecimalOrZero(price)
	}
	sort.SliceStable(ids, func(i, j int) bool {
		a := items[ids[i]]
		b := items[ids[j]]
		priceCmp := prices[ids[i]].Cmp(prices[ids[j]])
		if priceCmp != 0 {
			if buy {
				return priceCmp > 0
			}
...
func matchLimitOrderAmount(buy, sell *InvokeItem, price *scommon.Decimal) (*scommon.Decimal, int64, error) {
	...
	expected, err := limitOrderItemExpectedAmount(buy)
	if err != nil {
		return nil, 0, err
	}
	bought := buy.OutAmt
...
func applyLimitOrderDeal(buy, sell *InvokeItem, matchAmt *scommon.Decimal, matchValue int64, sellPrice string) error {
	...
	buyFinished, err := limitOrderBuyFinished(buy, sellPrice)
	if err != nil {
		return err
	}
	if buyFinished {
	...
	sellFinished, err := limitOrderSellFinished(sell)
	if err != nil {
		return err
	}
	if sellFinished {
...
func limitOrderSellFinished(sell *InvokeItem) (bool, error) {
	...
	price, err := limitOrderItemPrice(sell)
	if err != nil {
		return false, err
	}
	unitPrice := parseDecimalOrZero(price)
	...
	value, err := scommon.DecimalMul(remaining, unitPrice).FloorInt64()
	if err != nil {
		return false, fmt.Errorf("limit order remaining value: %w", err)
	}
	return value == 0, nil
}
...
func applyInvalidItems(contract Contract, state *TemplateRuntimeState, plan *SettlementPlan, height int64) (bool, error) {
	return applyInvalidItemsInRange(contract, state, plan, -1<<63, height)
}

func applyInvalidItemsInRange(contract Contract, state *TemplateRuntimeState, plan *SettlementPlan, firstHeight, height int64) (bool, error) {
	changed := false
	for i := range state.Items {
		item := &state.Items[i]
		if item.Finished() || item.Reason != InvokeReasonInvalid || item.Height < firstHeight || item.Height > height {
			continue
		}
		addSettlementInputs(plan, item)
		transfers, err := invalidRefundTransfers(contract, item)
		if err != nil {
			return false, err
		}
		plan.Transfers = append(plan.Transfers, transfers...)
...
func invalidRefundTransfers(contract Contract, item *InvokeItem) ([]SettlementTransfer, error) {
	if item == nil {
		return nil, nil
	}
	...
	if item.RetainedAssetB != nil && item.RetainedAssetB.Sign() > 0 && assetB != "" {
		if assetB == SatoshiAssetName {
			value, err := decimalInt64(item.RetainedAssetB)
			if err != nil {
				return nil, fmt.Errorf("invalid refund satoshi amount: %w", err)
			}
			out = append(out, SettlementTransfer{
				ItemID:   item.ID,
				To:       item.Address,
...
func applyCloseItems(contract Contract, state *TemplateRuntimeState, plan *SettlementPlan, height int64, deployer string) (bool, error) {
	...
	for i := range state.Items {
		...
		if amm, ok := contract.(*AMMContract); ok {
			if err := appendAMMLPCloseTransfers(state, plan, closeItem, amm.AssetName); err != nil {
				return false, err
			}
...
func appendAMMLPCloseTransfers(state *TemplateRuntimeState, plan *SettlementPlan, item *InvokeItem, assetName string) error {
	if state == nil || plan == nil {
		return nil
	}
	running := state.AMMData()
	...
	poolGas, err := decimalInt64(running.AssetBInPool)
	if err != nil {
		return fmt.Errorf("AMM close pool gas: %w", err)
	}
...
func refundMatchesItem(refund *InvokeItem, itemID int64) (bool, error) {
	...
	param, err := decodeRefundItemParam(refund)
	if err != nil {
		return false, err
	}
	...
	for _, targetID := range param.ItemIDs {
		if targetID == itemID {
			return true, nil
		}
	}
...
func settleAMMBuy(item *InvokeItem, poolAsset *scommon.Decimal, poolGas int64, availableAsset *scommon.Decimal, k *scommon.Decimal,
	...
	minAsset, err := limitOrderItemExpectedAmount(item)
	if err != nil {
		return SettlementDeal{}, nil, false, err
	}
	inputValue := maxInputValue
	calculatedOut, err := calculateAMMBuyOutput(inputValue, poolAsset, poolGas)
	if err != nil {
		return SettlementDeal{}, nil, false, err
	}
...
func calculateAMMBuyOutput(inputValue int64, poolAsset *scommon.Decimal, poolGas int64) (*scommon.Decimal, error) {
	if poolAsset == nil || poolAsset.Value == nil || poolAsset.Sign() <= 0 || poolGas <= 0 || inputValue <= 0 {
		return parseDecimalOrZero("0"), nil
	}
	effectiveInput := new(big.Int).Mul(big.NewInt(inputValue), big.NewInt(1000-SwapServiceFeeRatio))
	numerator := new(big.Int).Mul(new(big.Int).Set(poolAsset.Value), effectiveInput)
	denominator := new(big.Int).Mul(big.NewInt(poolGas), big.NewInt(1000))
	denominator.Add(denominator, effectiveInput)
	if denominator.Sign() <= 0 {
		return nil, fmt.Errorf("failed to calculate AMM buy output")
	}
	return &scommon.Decimal{
		Precision: poolAsset.Precision,
		Value:     new(big.Int).Quo(numerator, denominator),
	}, nil
}

func calculateAMMSellOutput(inAsset, poolAsset *scommon.Decimal, poolGas int64) (int64, error) {
	if inAsset == nil || inAsset.Value == nil || poolAsset == nil || poolAsset.Value == nil ||
		inAsset.Sign() <= 0 || poolAsset.Sign() <= 0 || poolGas <= 0 {
		return 0, nil
	}
	precision := poolAsset.Precision
	if inAsset.Precision > precision {
		precision = inAsset.Precision
	}
	poolUnits := poolAsset.NewPrecision(precision).Value
	inputUnits := inAsset.NewPrecision(precision).Value
	effectiveInput := new(big.Int).Mul(inputUnits, big.NewInt(1000-SwapServiceFeeRatio))
	numerator := new(big.Int).Mul(big.NewInt(poolGas), effectiveInput)
	denominator := new(big.Int).Mul(poolUnits, big.NewInt(1000))
	denominator.Add(denominator, effectiveInput)
	if denominator.Sign() <= 0 {
		return 0, fmt.Errorf("failed to calculate AMM sell output")
	}
	out := new(big.Int).Quo(numerator, denominator)
	if !out.IsInt64() {
		return 0, fmt.Errorf("AMM sell output overflows int64")
	}
...
func settleAMMSell(item *InvokeItem, poolAsset *scommon.Decimal, poolGas int64, availableGas int64, k *scommon.Decimal) (SettlementDeal, SettlementTransfer, bool, error) {
	...
	outGas, err := calculateAMMSellOutput(inAsset, poolAsset, poolGas)
	if err != nil {
	...
	minGas, err := limitOrderItemExpectedAmount(item)
	if err != nil {
		return SettlementDeal{}, SettlementTransfer{}, false, err
	}
	if minGas.Sign() > 0 && scommon.NewDefaultDecimal(outGas).Cmp(minGas) < 0 {
		item.Reason = InvokeReasonSlippageProtect
		transfer := markItemRefunded(item)
		return SettlementDeal{}, transfer, false, nil
	}
...
func reserveAMMLiquidity(addAsset *scommon.Decimal, addGas int64, poolAsset *scommon.Decimal, poolGas int64, running AMMRunningData) (*scommon.Decimal, int64, *scommon.Decimal, int64, error) {
	if addAsset == nil || addAsset.Sign() <= 0 || addGas <= 0 {
		return parseDecimalOrZero("0"), 0, parseDecimalOrZero("0"), 0, nil
	}
	if poolAsset == nil || poolAsset.Sign() <= 0 || poolGas <= 0 || !running.TradingReady {
		return addAsset, addGas, parseDecimalOrZero("0"), 0, nil
	}
	...
	if reserveAsset == nil {
		return parseDecimalOrZero("0"), 0, addAsset, addGas, nil
	}
	reserveAsset = reserveAsset.NewPrecision(addAsset.Precision)
	if reserveAsset.Cmp(addAsset) > 0 {
		reserveAsset = addAsset
		var err error
		reserveGas, err = scommon.DecimalMul(addAsset, price).CeilInt64()
		if err != nil {
			return nil, 0, nil, 0, fmt.Errorf("AMM liquidity reserve gas: %w", err)
		}
	...
	return reserveAsset, reserveGas, leftAsset, leftGas, nil
}
...
func splitAMMRemoveLiquidity(asset *scommon.Decimal, gas int64, depositValue int64) (*scommon.Decimal, int64, *scommon.Decimal, int64, error) {
	...
	if !foundationProfitBig.IsInt64() {
		return nil, 0, nil, 0, fmt.Errorf("AMM foundation profit overflows int64")
	}
	...
	if !foundationGasBig.IsInt64() {
		return nil, 0, nil, 0, fmt.Errorf("AMM foundation gas overflows int64")
	}
