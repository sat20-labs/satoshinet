...
func TestDeriveCreateContractAddress(t *testing.T) {
	...
	require.Equal(t, EVMAddressFromGeth(crypto.CreateAddress(GethAddress(caller), 3)), contract.Hash)
	require.Equal(t, gethcommon.Address(contract.Hash).Hex(), ContractGethAddress(contract).Hex())
}
