...
func TestEVMEndToEndDeployInvokeReplayAndReorg(t *testing.T) {
	...
	deployResultTx, err := evm.BuildCanonicalResultTx(evm.CanonicalResultTxRequest{
	...
	require.NoError(t, err)
	invokeResultTx, err := evm.BuildCanonicalResultTx(evm.CanonicalResultTxRequest{
		Status:    evm.ResultStatusSuccess,
	...
	resultVerifier := evm.CanonicalResultVerifier{
		...
		ResolveOutput: func(tx *wire.MsgTx) ([]evm.ResultOutput, error) {
			return evm.ResultOutputsFromTx(
				tx, evm.TestnetContractPrefix, e2ERecipientResolver,
			)
		},
	}
	...
	store := blockchain.NewEVMStateStore(e2EDatabase(t))
...
func TestEVMEndToEndDecimalAssetTransfer(t *testing.T) {
	...
	resultTx, err := evm.BuildCanonicalResultTx(evm.CanonicalResultTxRequest{
		Status:    evm.ResultStatusSuccess,
		Records:   pending,
		GasConfig: cfg,
		UTXOs: func(got evm.ContractAddress) ([]evm.UTXO, error) {
	...
	require.NoError(t, err)
	outputs, err := evm.ResultOutputsFromTx(resultTx, evm.TestnetContractPrefix, e2ERecipientResolver)
	require.NoError(t, err)
	...
	verifier := evm.CanonicalResultVerifier{
		...
		ResolveOutput: func(tx *wire.MsgTx) ([]evm.ResultOutput, error) {
			return evm.ResultOutputsFromTx(tx, evm.TestnetContractPrefix, e2ERecipientResolver)
		},
	}
	replayRuntime := evm.NewRuntime(nil)
