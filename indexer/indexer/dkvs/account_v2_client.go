...
// VerifyAccountRecordForClient verifies an account-scoped version-2 record
// without relying on server-side permission checks or an embedded public key.
...
// AssembleAccountBlobFromRecords verifies and assembles a version-2 blob whose
// ownership is derived from its /blob/<account_id>/... keys.
