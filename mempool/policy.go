...
func CheckTransactionStandard(tx *btcutil.Tx, height int32,
	...
	templateInfo, err := tmplcontract.ClassifyTxForBlockOrder(msgTx, tmplcontract.TestnetContractPrefix)
	if err == nil && templateInfo.IsTemplate {
		switch templateInfo.Type {
		case tmplcontract.TxTypeDeploy:
			if !hasEVMContractOutput {
				return txRuleError(wire.RejectNonstandard,
					"TEMPLATE_DEPLOY has no contract funding output")
			}
		case tmplcontract.TxTypeInvoke:
			if !hasEVMContractOutput {
				return txRuleError(wire.RejectNonstandard,
					"TEMPLATE_INVOKE has no contract funding output")
			}
		case tmplcontract.TxTypeResult:
			return txRuleError(wire.RejectNonstandard,
				"TEMPLATE_RESULT transactions are generated by mining")
		}
	} else {
