...
const (
	...
	// maxStandardNullDataOutputs is the maximum number of OP_RETURN outputs
	// allowed for ordinary mempool transactions.  Contract deploy payloads
	// may exceed this because large EVM init code is split across multiple
	// contract deploy OP_RETURN outputs.
	maxStandardNullDataOutputs = 4
)
...
func CheckTransactionStandardWithParams(tx *btcutil.Tx, height int32,
	...
	if found {
		...
		case contractcommon.TxTypeResult:
			return txRuleError(wire.RejectNonstandard,
				"contract RESULT transactions are generated by mining")
		}
	}
	if err := checkNullDataOutputCount(msgTx, numNullDataOutputs); err != nil {
		return err
	}
...
func checkNullDataOutputCount(msgTx *wire.MsgTx, numNullDataOutputs int) error {
	if numNullDataOutputs <= maxStandardNullDataOutputs {
		return nil
	}
	for _, txOut := range msgTx.TxOut {
		if txOut == nil || len(txOut.PkScript) == 0 ||
			txOut.PkScript[0] != txscript.OP_RETURN {
			continue
		}
		txType, _, err := contractcommon.ReadNullDataScript(txOut.PkScript)
		if err != nil || txType != contractcommon.TxTypeDeploy {
			return txRuleError(wire.RejectNonstandard,
				fmt.Sprintf("transaction has %d OP_RETURN outputs; only contract deploy transactions may exceed %d",
					numNullDataOutputs, maxStandardNullDataOutputs))
		}
	}
	return nil
}
